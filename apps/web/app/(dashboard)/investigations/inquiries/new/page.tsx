...
export default function NewInquiryPage() {
  ...
  const [matchAllSources, setMatchAllSources] = React.useState(true);
  const [selectedSources, setSelectedSources] = React.useState<Set<string>>(new Set());
  const [selectedDetectors, setSelectedDetectors] = React.useState<Set<string>>(new Set());
  const [selectedCustomKeys, setSelectedCustomKeys] = React.useState<Set<string>>(new Set());
  ...
  // Load filter options; re-scope finding types to the chosen sources.
  ...
  // Live preview (debounced) so the user sees what will land before creating.
  ...
  return (
    <div className="container max-w-5xl py-8 space-y-6">
      <div>
        <Button variant="outline" onClick={() => router.push("/investigations")}
          className="mb-4 rounded-[4px] border-2 border-border shadow-[3px_3px_0_var(--color-border)]">
          <ArrowLeft className="mr-2 h-4 w-4" /> Investigations
        </Button>
        <h1 className="font-serif text-3xl font-black uppercase tracking-[0.08em]">New inquiry</h1>
        <p className="text-muted-foreground mt-2 max-w-2xl">
          An inquiry is a saved query. Findings that match it are tracked so you can pull them into a case.
          Tune the filters and preview what currently matches before saving.
        </p>
      </div>
      ...
      <div className="flex gap-8 lg:gap-12">
        ...
        <aside className="hidden self-start md:sticky md:top-6 md:block md:w-48 space-y-4">
          ...
          <Button className="w-full rounded-[4px] border-2 border-border shadow-[3px_3px_0_var(--color-border)]"
            onClick={create} disabled={!title.trim() || saving}>
            {saving ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : null} Create inquiry
          </Button>
