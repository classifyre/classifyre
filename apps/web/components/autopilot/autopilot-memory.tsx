...
type Kind =
  ...
  | "DETECTOR_INSIGHT"
  | "OPERATOR_DIRECTIVE";
type ViewKind = Kind | "ALL" | "GLOSSARY";
...
export function AutopilotMemory() {
  ...
  const [kind, setKind] = React.useState<ViewKind>("ALL");
  ...
  const KINDS: Array<{
    ...
    () => [
      {
        value: "ALL",
        label: t("investigations.autopilot.memory.filterAll"),
        icon: <Brain className="h-3 w-3" />,
      },
      {
        value: "GLOSSARY",
        label: t("investigations.autopilot.memory.filterGlossary"),
        icon: <BookOpenText className="h-3 w-3" />,
      },
      {
        value: "DECISION_PRECEDENT",
        label: t("investigations.autopilot.memory.filterPrecedents"),
        icon: <Scale className="h-3 w-3" />,
      },
      {
        value: "ENTITY_MAP",
        label: t("investigations.autopilot.memory.filterEntityMap"),
        icon: <Table2 className="h-3 w-3" />,
      },
      {
        value: "SOURCE_PROFILE",
        label: t("investigations.autopilot.memory.filterSourceProfile"),
        icon: <Eye className="h-3 w-3" />,
      },
      {
        value: "DETECTOR_INSIGHT",
        label: t("investigations.autopilot.memory.filterDetectorInsight"),
        icon: <Search className="h-3 w-3" />,
      },
      {
        value: "OPERATOR_DIRECTIVE",
        label: t("investigations.autopilot.memory.filterOperatorDirective"),
    ...
    (kind: string): string => {
      switch (kind) {
        case "DECISION_PRECEDENT":
          return t("investigations.autopilot.memory.kindPrecedent");
        case "ENTITY_MAP":
          return t("investigations.autopilot.memory.kindEntityMap");
        case "SOURCE_PROFILE":
          return t("investigations.autopilot.memory.kindSourceProfile");
        case "DETECTOR_INSIGHT":
          return t("investigations.autopilot.memory.kindDetectorInsight");
        case "OPERATOR_DIRECTIVE":
          return t("investigations.autopilot.memory.kindOperatorDirective");
        default:
          return kind;
      }
    },
    ...
    try {
      ...
      toast.error(
        err instanceof Error
          ? err.message
          : t("investigations.autopilot.memory.toastLoadError"),
      );
    } finally {
      setLoading(false);
    }
  ...
  const remove = async (m: AgentMemoryDto) => {
    try {
      ...
      toast.error(
        err instanceof Error
          ? err.message
          : t("investigations.autopilot.memory.toastDeleteError"),
      );
    }
  };
  ...
  return (
    <div className="space-y-4">
          ...
          <>
            <div className="relative ml-auto w-56">
              ...
              <Input
                ...
                placeholder={t(
                  "investigations.autopilot.memory.searchPlaceholder",
                )}
            ...
            <Button size="sm" onClick={() => setAdding(true)}>
              ...
              {t("investigations.autopilot.memory.teach")}
            </Button>
          </>
        )}
      </div>
      ...
      {kind === "GLOSSARY" ? (
        <GlossaryWorkspace embedded />
      ) : loading ? (
        <div className="text-muted-foreground flex items-center justify-center gap-2 py-12 text-sm">
          <Loader2 className="h-4 w-4 animate-spin" />{" "}
          {t("investigations.autopilot.memory.loading")}
        </div>
      ) : items.length === 0 ? (
        <EmptyState
          icon={Brain}
        ...
        <>
          <p className="font-mono text-[11px] text-muted-foreground">
            {t("investigations.autopilot.memory.entryCount", {
              count: String(total),
              suffix: total === 1 ? "y" : "ies",
            })}
          </p>
              ...
              >
                ...
                <div className="mt-2 flex items-center gap-1.5">
                  {m.tags.slice(0, 4).map((t) => (
                    <span
                      key={t}
                      className="rounded bg-muted/60 px-1.5 py-px font-mono text-[9px]"
                    >
                      {t}
                    </span>
                  ))}
                  {m.refType === "case" && m.refId && (
                    <Link
                      href={nsHref(`/investigations/${m.refId}`)}
                      className="font-mono text-[9px] uppercase tracking-wide text-[#d97706] hover:underline"
                    >
                      {t("investigations.autopilot.memory.openCase")}
                    </Link>
                  )}
                  {m.refType === "inquiry" && m.refId && (
                    <Link
                      href={nsHref(`/investigations/inquiries/${m.refId}`)}
                      className="font-mono text-[9px] uppercase tracking-wide text-[#d97706] hover:underline"
                    >
                      {t("investigations.autopilot.memory.openInquiry")}
                    </Link>
                  )}
      ...
      <MemoryAddDialog
        open={adding}
        defaultKind={
          kind === "ALL" || kind === "GLOSSARY" ? "OPERATOR_DIRECTIVE" : kind
        }
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.error(
        err instanceof Error
          ? err.message
          : t("investigations.autopilot.memory.toastUpdateError"),
      );
    } finally {
      setSaving(false);
    }
          ...
          <div className="grid grid-cols-[1fr_90px] gap-2">
            <div className="space-y-1">
              <Label className="text-[10px] font-mono uppercase tracking-wider">
                {t("investigations.autopilot.memory.labelTags")}
              </Label>
              <Input
                value={tags}
                onChange={(e) => setTags(e.target.value)}
                placeholder={t(
                  "investigations.autopilot.memory.placeholders.tags",
                )}
            ...
            <div className="space-y-1">
              <Label className="text-[10px] font-mono uppercase tracking-wider">
                {t("investigations.autopilot.memory.labelWeight")}
              </Label>
              <Input
                type="number"
                min={0}
  ...
  const save = async () => {
    try {
      ...
      toast.error(
        err instanceof Error
          ? err.message
          : t("investigations.autopilot.memory.toastAddError"),
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="rounded-[6px] border-2 border-border sm:max-w-md">
        <DialogHeader>
          <DialogTitle>
            {t("investigations.autopilot.memory.addDialogTitle")}
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-[150px_1fr] gap-2">
            <div className="space-y-1">
              <Label className="text-[10px] font-mono uppercase tracking-wider">
                {t("investigations.autopilot.memory.labelKind")}
              </Label>
              <Select value={kind} onValueChange={(v) => setKind(v as Kind)}>
                ...
                <SelectContent>
                  <SelectItem value="DECISION_PRECEDENT">
                    {t("investigations.autopilot.memory.kindPrecedent")}
                  </SelectItem>
                  <SelectItem value="ENTITY_MAP">
                    {t("investigations.autopilot.memory.kindEntityMap")}
                  </SelectItem>
                  <SelectItem value="SOURCE_PROFILE">
                    {t("investigations.autopilot.memory.kindSourceProfile")}
                  </SelectItem>
                  <SelectItem value="DETECTOR_INSIGHT">
                    {t("investigations.autopilot.memory.kindDetectorInsight")}
                  </SelectItem>
                  <SelectItem value="OPERATOR_DIRECTIVE">
                    {t("investigations.autopilot.memory.kindOperatorDirective")}
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-1">
              <Label className="text-[10px] font-mono uppercase tracking-wider">
                {t("investigations.autopilot.memory.labelKey")}
              </Label>
              <Input
                value={key}
                onChange={(e) => setKey(e.target.value)}
                placeholder={t(
                  "investigations.autopilot.memory.placeholders.key",
                )}
                maxLength={200}
                className="h-8 rounded-[4px] border-2 border-border text-sm"
              />
          ...
          <Textarea
            ...
            maxLength={2000}
            placeholder={t(
              "investigations.autopilot.memory.placeholders.content",
            )}
            className="rounded-[4px] border-2 border-border text-sm"
          />
          <div className="flex justify-end gap-2">
