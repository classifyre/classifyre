...
graph TD
    ...
    Ing -->|"/*"| Web
    Ing -->|"/api/*  /&lt;workspace&gt;/mcp  /socket.io/*"| API
...
Prefer reading first? The [chart README on GitHub](https://github.com/classifyre/classifyre/blob/main/helm/classifyre/README.md) documents every value, the sizing rules, and the production checklist in one page.
...
| Path | Target |
|---|---|
| `/` | Web UI (including `/<workspace>/…` app routes) |
| `/api/*` | REST API |
| `/<workspace>/mcp` | MCP protocol endpoint, scoped to one workspace |
| `/socket.io/*` | WebSocket |

<Callout type="info">
  **Workspace-scoped paths.** Application and MCP URLs carry the
  [workspace](/how-it-works/workspaces/) slug as their first segment — e.g.
  `https://<your-host>/acme-corp/findings` and
  `https://<your-host>/acme-corp/mcp`. The MCP rule is deliberately regex-based
  and does **not** rewrite the path, so the workspace segment reaches the API,
  which resolves the tenant from it. A bare `/mcp` with no workspace is not
  routed and returns `404`.
</Callout>
