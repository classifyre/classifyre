...
2. **From a fingerprint cluster** — In
   ...
   similarity cluster straight into a case; its members come in as evidence.
3. **Directly** — Use the "New case" button on the investigations page.
...
Each case has an AI mode setting that controls whether the
[Autopilot](/flow/investigations/autopilot/) case agent can autonomously operate
on this case, or only observe it. See
[Steering & Fine-Tuning](/flow/investigations/autopilot/steering/) for how
observe-only mode works at the instance, source, detector, and case level.
