...
Each custom detector runs one of six detection engines — plus **Tag**, which runs nothing at all and instead records a fact a [Custom connector](/sources/custom-connectors/) already knows. This page describes each and links to its configuration reference.
...
<CustomDetectorTypesGrid
  hrefBase="/detectors/custom-detectors/"
  variant="reference"
/>
...
| **Tag**                  | —        | No                        | Recording a fact the source system already holds  |
...
**Use a [Tag](/detectors/custom-detectors/tag/)** when there is nothing to detect — the source system already knows the answer, and a classifier could only re-derive it less reliably. A Tag detector is applied from a Custom connector notebook by its key, and does not appear in a source's detector list.

Classifyre creates semantic embeddings automatically during ingestion; they are
infrastructure rather than detector findings.
