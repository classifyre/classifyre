...
| `EXCLUSION` | **rule candidate** | Shared boilerplate is doing the matching | Don't grind the pairs — write an [exclusion](/duplicates/tuning/#exclusions) so the text stops driving matches at all |
...
The bulk action. It records **Confirm** on every undecided pair in this pattern,
inside the current cutoffs and lineage filter.
