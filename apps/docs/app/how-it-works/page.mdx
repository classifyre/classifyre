...
Everything below happens inside a **[workspace](/how-it-works/workspaces/)** —
an isolated investigation with its own sources, findings, cases, and AI memory.
One Classifyre instance can hold many of them (a client, a region, a business
unit), and nothing is shared between them.
...
graph TD
    ...
    Ranking --> Duplicates["Duplicate review<br/>shared-value links"]
    Inquiries --> Cases["Cases<br/>the investigation case file"]
    Duplicates --> Cases
...
**3. Detect.** **Detectors** — pattern rules, ML models, tags, or AI-based
custom detectors — read each asset and raise a **finding** whenever something's
worth a look, each carrying a **priority** (how much it matters, shown in the
API as `severity`) and a **confidence** (how sure the detector is).

**4. Rank.** Priority alone doesn't say what to look at *first*. A ranking
pass scores each finding's **importance** — how much it looks like a genuine
lead, independent of the priority the detector assigned — using signals like
recurrence across documents, context quality, and whether it looks like test
data. See [Ranking & the Semantic Layer](/how-it-works/ranking-and-semantics/).

**5. Watch and connect.** **Inquiries** are saved questions that keep watching
for matching findings; **duplicate matching** links assets sharing concrete values
like an email or ID — revealing when the same record shows up in several
systems. See [Connections & Duplicates](/how-it-works/duplicates-and-similarity/).
...
- **Priority is not importance.** Priority is what the detector asserted about
  a match — how much this kind of thing matters to you. Ranking says how much
  this particular finding, in context, looks like a genuine lead. A "critical"
  finding repeated as boilerplate everywhere isn't where you start.

  Priority is *not* a danger scale. A finding is critical because it is
  important to the question you are asking — a company approaching insolvency,
  a shipment routed to the wrong address, a credential where it should not be
  — not because it is dangerous. The stored values are still
  `CRITICAL … INFO`; only what they mean changed.
...
| [Ranking & the Semantic Layer](/how-it-works/ranking-and-semantics/) | Importance vs priority, recalibration. |
| [Connections & Duplicates](/how-it-works/duplicates-and-similarity/) | Shared-value matching vs semantic similarity, and what lineage adds. |
