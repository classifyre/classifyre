...
You can also point the source at a folder the scan can already see and read it
in place — the right choice for a dump too large to upload. That means a
bind-mounted directory with the [all-in-one Docker
image](/deployment/docker/), or one the chart mounts into CLI jobs via
`api.localFolders` on Kubernetes. Add it under **Local folders**, then:
