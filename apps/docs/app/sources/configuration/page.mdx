...
Every source also accepts an **augmentation** block: an optional Python notebook
that enriches each asset after extraction and before detection — extra metadata,
tags, links, a URN, and relationship edges. Additive only: it cannot change what
the connector extracted, drop an asset, or fail a scan. See
