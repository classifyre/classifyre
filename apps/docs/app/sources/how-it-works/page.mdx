...
## Change scheduling or sampling in bulk

On the **Sources** page, select individual rows or use the header checkbox to
select every source matching the current filters. **Update sources** opens a
preview where you can apply a new [schedule](/sources/testing/), a new
[sampling strategy](/sources/sampling/), or both.

Each section must be enabled explicitly. Sections you leave disabled — along
with connection details, credentials, detectors, optional settings, resources,
names, and descriptions — remain unchanged. This makes it safe to align a set
of sources without replacing the rest of their configuration.
