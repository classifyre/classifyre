...
| **Links** | Related items discovered on or around it — see [Lineage & Relationships](/sources/lineage/) for what those relationships mean. |
...
### The augmentation namespace

Values an [augmentation notebook](/sources/augmentation/) stages land under a
single reserved key — `metadata["augmentation"]` — alongside, never inside, the
connector's own fields. That namespace is exempt from metadata validation, so
enrichment can never read as contract drift.
...
- **It helps the AI.** When a source is silent, the [Config and Detector
  agents](/investigations/autopilot/agents/) read asset metadata to work out
  what kind of data they're looking at and which detectors to enable — the
  "cold start" described in [Autopilot](/investigations/autopilot/).
