...
describe('CasesService', () => {
  ...
  const mockPrisma = {
    ...
    inquiry: { updateMany: jest.fn(), findUnique: jest.fn(), findMany: jest.fn(), delete: jest.fn() },
    caseInquiry: { createMany: jest.fn(), findMany: jest.fn(), findUnique: jest.fn(), delete: jest.fn() },
  ...
  it('links inquiries when created with inquiryIds (m2m join rows)', async () => {
    ...
    mockPrisma.case.findUnique.mockResolvedValue({ ...caseRow(), evidence: [], inquiryLinks: [] });
  ...
  it('links additional inquiries to an existing case', async () => {
    mockPrisma.case.findUnique.mockResolvedValue({ ...caseRow(), evidence: [], inquiryLinks: [] });
    mockPrisma.inquiry.findMany.mockResolvedValue([{ id: 'q9', title: 'Extra' }]);
  ...
  it('unlinks an inquiry without touching the inquiry row', async () => {
    mockPrisma.case.findUnique.mockResolvedValue({ ...caseRow(), evidence: [], inquiryLinks: [] });
    mockPrisma.caseInquiry.findUnique.mockResolvedValue({ id: 'link1', inquiry: { title: 'Q' } });
    await service.unlinkInquiry('c1', 'q1');
    expect(mockPrisma.caseInquiry.delete).toHaveBeenCalledWith({ where: { id: 'link1' } });
    expect(mockPrisma.inquiry.delete).not.toHaveBeenCalled();
  });

  it('rejects creating a case with unknown inquiries', async () => {
    mockPrisma.inquiry.findMany.mockResolvedValue([]);
    await expect(service.create({ title: 'Case', inquiryIds: ['ghost'] })).rejects.toBeInstanceOf(
      BadRequestException,
    );
  });
  ...
  it('batch-attaches findings, creating asset evidence as needed', async () => {
    mockPrisma.case.findUnique.mockResolvedValue({ id: 'c1' });
    mockPrisma.finding.findMany.mockResolvedValue([
      { id: 'f1', assetId: 'a1', findingType: 'ssn', severity: 'HIGH', detectorType: 'PII', customDetectorName: null, matchedContent: 'x', asset: { name: 'a.csv', assetType: 'file', sourceType: 'S3_COMPATIBLE_STORAGE' } },
    ]);
    mockPrisma.caseEvidence.upsert.mockResolvedValue({ id: 'ev1' });
  ...
  it('closes a case and archives only inquiries with no other open case', async () => {
    mockPrisma.case.findUnique.mockResolvedValue({ ...caseRow({ status: 'CLOSED', conclusion: 'It was the wiki.' }), evidence: [], inquiryLinks: [] });
    ...
    mockPrisma.inquiry.findMany.mockResolvedValue([
      ...
      { id: 'q2', caseLinks: [{ case: { id: 'c1', status: 'CLOSED' } }, { case: { id: 'c2', status: 'OPEN' } }] },
    ]);
    mockPrisma.inquiry.updateMany.mockResolvedValue({ count: 1 });

    const result = await service.close('c1', { conclusion: 'It was the wiki.' });
  ...
  it('refuses to close a case without a conclusion', async () => {
    mockPrisma.case.findUnique.mockResolvedValue({ id: 'c1' });
    await expect(service.close('c1', { conclusion: '   ' })).rejects.toBeInstanceOf(BadRequestException);
