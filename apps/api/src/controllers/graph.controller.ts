...
export class GraphController {
  ...
  @ApiOperation({ summary: 'Expand the graph around a seed entity (recursive traversal)' })
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Named pivot question on a node (e.g. who_touched, upstream_lineage, emails)',
  })
  @ApiResponse({ status: 200, type: GraphResponseDto })
  ...
  async ingestEdges(@Body() dto: BulkIngestEdgesDto): Promise<BulkIngestEdgesResponseDto> {
    return this.graphService.upsertEdges(dto);
  }

  @Post('rebuild-edges')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Rebuild all inferred edges from existing assets and findings' })
  ...
  @Get('relation-types')
  @ApiOperation({ summary: 'Get all relation types in use + vocabulary suggestions' })
  ...
  @Post('edges/manual')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Create a manual edge between two entities (user-defined relation type)' })
  @ApiResponse({ status: 200, type: EdgeDetailDto })
  async createManualEdge(@Body() dto: CreateManualEdgeDto): Promise<EdgeDetailDto> {
  ...
  @ApiResponse({ status: 200, type: EdgeDetailDto })
  async updateEdge(@Param('id') id: string, @Body() dto: UpdateEdgeDto): Promise<EdgeDetailDto> {
