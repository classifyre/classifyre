...
type HypothesisRow = Prisma.HypothesisGetPayload<{ include: { support: true } }>;

@Injectable()
export class HypothesesService {
  constructor(private readonly prisma: PrismaService) {}

  private readonly include = { support: true } satisfies Prisma.HypothesisInclude;

  async list(caseId: string): Promise<HypothesisResponseDto[]> {
    const rows = await this.prisma.hypothesis.findMany({ where: { caseId }, include: this.include, orderBy: { createdAt: 'asc' } });
  ...
  async create(caseId: string, dto: CreateHypothesisDto): Promise<HypothesisResponseDto> {
    ...
    const created = await this.prisma.hypothesis.create({
      data: { caseId, statement: dto.statement, status: dto.status, confidence: dto.confidence, createdBy: dto.createdBy },
  ...
  async update(id: string, dto: UpdateHypothesisDto): Promise<HypothesisResponseDto> {
    ...
    const updated = await this.prisma.hypothesis.update({
      where: { id },
      data: { statement: dto.statement, status: dto.status, confidence: dto.confidence, color: dto.color },
      include: this.include,
    });
  ...
  async linkSupport(hypothesisId: string, dto: LinkSupportDto): Promise<HypothesisResponseDto> {
    const hypothesis = await this.prisma.hypothesis.findUnique({ where: { id: hypothesisId }, select: { caseId: true } });
    ...
    await this.prisma.hypothesisSupport.upsert({
      where: { hypothesisId_targetType_targetId: { hypothesisId, targetType: dto.targetType, targetId: dto.targetId } },
      create: {
        hypothesisId,
  ...
  async unlinkSupport(hypothesisId: string, linkId: string): Promise<HypothesisResponseDto> {
    const link = await this.prisma.hypothesisSupport.findUnique({ where: { id: linkId } });
  ...
  private async validateTarget(targetType: 'evidence' | 'finding', targetId: string, caseId: string): Promise<void> {
    if (targetType === 'evidence') {
      ...
      if (!ev || ev.caseId !== caseId) throw new BadRequestException('Evidence must belong to the same case as the hypothesis');
    } else {
      const cf = await this.prisma.caseFinding.findUnique({ where: { id: targetId }, select: { caseId: true } });
      if (!cf || cf.caseId !== caseId) throw new BadRequestException('Finding must belong to the same case as the hypothesis');
    }
  }
