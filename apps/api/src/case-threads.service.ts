...
import { CaseActivityType, CaseThreadEntryType, CaseThreadKind, EvidenceStance, HypothesisStatus, Prisma } from '@prisma/client';
...
type ThreadRow = Prisma.CaseThreadGetPayload<{ include: { support: true; entries: true } }>;
...
export class CaseThreadsService {
  ...
  private readonly include = { support: true, entries: { orderBy: { createdAt: 'asc' as const } } } satisfies Prisma.CaseThreadInclude;
  ...
  async create(caseId: string, dto: CreateThreadDto): Promise<ThreadResponseDto> {
    ...
    const thread = await this.prisma.$transaction(async (tx) => {
      const created = await tx.caseThread.create({
        data: {
          ...
          title: dto.title,
          status: dto.kind === CaseThreadKind.DISCUSSION ? null : (dto.status ?? HypothesisStatus.PROPOSED),
      ...
      if (dto.statement) {
        await tx.caseThreadEntry.create({
          data: { threadId: created.id, entryType: CaseThreadEntryType.STATEMENT, body: dto.statement, author: dto.createdBy },
        });
      }
      await this.activity.record(
        caseId,
        CaseActivityType.THREAD_CREATED,
        { threadId: created.id, threadTitle: created.title, kind: created.kind },
        dto.createdBy,
        tx,
      );
      return created;
  ...
  async update(id: string, dto: UpdateThreadDto): Promise<ThreadResponseDto> {
    const existing = await this.prisma.caseThread.findUnique({ where: { id }, select: { id: true, caseId: true, status: true, confidence: true } });
    ...
    const entriesToCreate: Array<{ entryType: CaseThreadEntryType; metadata: Record<string, unknown> }> = [];
    ...
    if (dto.confidence !== undefined && dto.confidence !== Number(existing.confidence)) {
      data.confidence = dto.confidence;
      entriesToCreate.push({
        entryType: CaseThreadEntryType.CONFIDENCE_CHANGE,
        metadata: { previousConfidence: existing.confidence === null ? null : Number(existing.confidence), confidence: dto.confidence },
      });
      activityPayload.confidence = dto.confidence;
    }

    const primaryActivityType = entriesToCreate.some((e) => e.entryType === CaseThreadEntryType.STATUS_CHANGE)
      ? CaseActivityType.THREAD_STATUS_CHANGED
      : entriesToCreate.some((e) => e.entryType === CaseThreadEntryType.CONFIDENCE_CHANGE)
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      for (const entry of entriesToCreate) {
        await tx.caseThreadEntry.create({ data: { threadId: id, entryType: entry.entryType, metadata: entry.metadata as JsonInput } });
      }
      if (primaryActivityType) {
        await this.activity.record(existing.caseId, primaryActivityType, activityPayload, dto.actor, tx);
      }
    });
  ...
  async remove(id: string): Promise<void> {
    const existing = await this.prisma.caseThread.findUnique({ where: { id }, select: { id: true, caseId: true, title: true, kind: true } });
    if (!existing) throw new NotFoundException(`Thread ${id} not found`);
    await this.prisma.caseThread.delete({ where: { id } });
  }

  async addEntry(threadId: string, dto: AddThreadEntryDto): Promise<ThreadResponseDto> {
    const thread = await this.prisma.caseThread.findUnique({ where: { id: threadId }, select: { id: true, caseId: true, title: true } });
    if (!thread) throw new NotFoundException(`Thread ${threadId} not found`);

    await this.prisma.$transaction(async (tx) => {
      const entry = await tx.caseThreadEntry.create({
        data: { threadId, entryType: dto.entryType, body: dto.body, author: dto.author },
      });
      ...
      await this.activity.record(
        thread.caseId,
        actType,
        { threadId, entryId: entry.id, threadTitle: thread.title, entryType: dto.entryType, body: dto.body?.slice(0, 100) },
        dto.author,
        tx,
      );

      if (dto.entryType === CaseThreadEntryType.STATEMENT && dto.body) {
        await tx.caseThread.update({ where: { id: threadId }, data: { title: dto.body.slice(0, 200) } });
      }
    });
  ...
  async getEntries(threadId: string, cursor?: string, limit = 50): Promise<ThreadEntriesResponseDto> {
    const found = await this.prisma.caseThread.findUnique({ where: { id: threadId }, select: { id: true } });
  ...
  async linkSupport(threadId: string, dto: LinkThreadSupportDto): Promise<ThreadResponseDto> {
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.caseThreadSupport.upsert({
        where: { threadId_targetType_targetId: { threadId, targetType: dto.targetType, targetId: dto.targetId } },
        create: { threadId, targetType: dto.targetType, targetId: dto.targetId, stance: dto.stance ?? EvidenceStance.SUPPORTS, weight: dto.weight, note: dto.note },
        update: { stance: dto.stance ?? EvidenceStance.SUPPORTS, weight: dto.weight, note: dto.note },
      });
      await this.activity.record(
        thread.caseId,
  ...
  private async getOne(id: string): Promise<ThreadResponseDto> {
    const row = await this.prisma.caseThread.findUnique({ where: { id }, include: this.include });
  ...
  private async mapMany(rows: ThreadRow[]): Promise<ThreadResponseDto[]> {
    const evidenceIds = rows.flatMap((r) => r.support).filter((s) => s.targetType === 'evidence').map((s) => s.targetId);
    const findingIds = rows.flatMap((r) => r.support).filter((s) => s.targetType === 'finding').map((s) => s.targetId);

    const [evidenceRows, findingRows] = await Promise.all([
      evidenceIds.length > 0
        ? this.prisma.caseEvidence.findMany({ where: { id: { in: evidenceIds } }, select: { id: true, label: true, entityId: true } })
        : Promise.resolve([]),
      findingIds.length > 0
        ? this.prisma.caseFinding.findMany({ where: { id: { in: findingIds } }, select: { id: true, label: true } })
        : Promise.resolve([]),
    ]);

    const evidenceMap = new Map<string, string>(evidenceRows.map((e): [string, string] => [e.id, e.label ?? e.entityId]));
    const findingMap = new Map<string, string>(findingRows.map((cf): [string, string] => [cf.id, cf.label]));
    ...
    const labelFor = (targetType: string, targetId: string): string => {
      ...
      if (targetType === 'finding') return findingMap.get(targetId) ?? '(deleted finding)';
      return targetId;
    };
