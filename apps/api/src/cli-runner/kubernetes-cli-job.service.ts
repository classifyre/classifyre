...
export class KubernetesCliJobService {
  ...
  private async buildJobFromTemplate(
    ...
    // A connection test or notebook execution answers in seconds and holds no
    // corpus, but both are built from the extract template: a 3-second test
    // waited 6.5 minutes Pending for a 7 GiB slot behind a running scan
    // (GENESIS field report P2), and a notebook inherits the same 7 GiB until
    // given its own profile. Both get the small profile, request == limit so
    // the scheduler can place them next to an extract. Recipe `resources`
    // below can still raise it for one source.
    if (params.mode === 'test' || params.mode === 'notebook') {
      const memory =
        process.env.K8S_CLI_TEST_JOB_MEMORY ||
        process.env.K8S_CLI_QUICK_JOB_MEMORY ||
        '2Gi';
      const cpu =
        process.env.K8S_CLI_TEST_JOB_CPU ||
        process.env.K8S_CLI_QUICK_JOB_CPU ||
        '1';
      const cpuRequest =
        process.env.K8S_CLI_TEST_JOB_CPU_REQUEST ||
        process.env.K8S_CLI_QUICK_JOB_CPU_REQUEST ||
        '250m';
      container.resources = {
        requests: { memory, cpu: cpuRequest },
        limits: { memory, cpu },
      };
    }
