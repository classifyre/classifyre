...
function fetcher(edges: E[]) {
  ...
  const calls: string[][] = [];
  const fetchLevel = async (frontier: TraceNodeRef[]): Promise<TraceEdgeRow[]> => {
    calls.push(frontier.map((f) => f.id));
    const out: TraceEdgeRow[] = [];
    for (const f of frontier) {
      for (const r of rows) {
        if (r.from_id === f.id) out.push({ ...r, src_type: 'asset', src_id: f.id, dir: 'out' });
        if (r.to_id === f.id) out.push({ ...r, src_type: 'asset', src_id: f.id, dir: 'in' });
      }
    }
    return out;
  };
...
describe('nextSide', () => {
  it('sends lineage into the seed upstream and out of it downstream', () => {
    ...
    expect(nextSide('seed', { dir: 'out', kind: 'lineage' }, 'both')).toBe('down');
  });
  ...
  it('keeps upstream going up and downstream going down', () => {
    ...
    expect(nextSide('down', { dir: 'out', kind: 'lineage' }, 'both')).toBe('down');
  ...
  it('puts duplicates and similar assets beside the node they hang off', () => {
    expect(nextSide('seed', { dir: 'out', kind: 'duplicates' }, 'both')).toBe('side');
...
describe('walkTrace', () => {
  ...
  it('walks upstream, downstream and sideways, hop by hop, without doubling back', async () => {
    ...
    const walk = await walkTrace(seedA, { depth: 2, direction: 'both', limit: 100 }, fetchLevel);
    const side = (id: string) => walk.found.get(traceKey('asset', id));
    expect(side('A')).toMatchObject({ side: 'seed', depth: 0 });
    expect(side('U')).toMatchObject({ side: 'up', depth: 1, via: 'asset:A', viaKind: 'lineage' });
    ...
    expect(side('S')).toMatchObject({ side: 'side', depth: 1, viaKind: 'duplicates' });
  ...
  it('stops at the requested depth', async () => {
    ...
    const walk = await walkTrace(seedA, { depth: 1, direction: 'both', limit: 100 }, fetchLevel);
  ...
  it('traces one way when asked', async () => {
    const { fetchLevel } = fetcher(graph);
    const walk = await walkTrace(seedA, { depth: 3, direction: 'down', limit: 100 }, fetchLevel);
  ...
  it('marks the walk truncated at its node limit, and only returns edges between kept nodes', async () => {
    const { fetchLevel } = fetcher(graph);
    const walk = await walkTrace(seedA, { depth: 3, direction: 'both', limit: 3 }, fetchLevel);
  ...
  it('walks from several seeds at once', async () => {
    ...
    expect(walk.found.get(traceKey('asset', 'U'))).toMatchObject({ side: 'down', via: 'asset:U2' });
    expect(walk.found.get(traceKey('asset', 'D'))).toMatchObject({ side: 'up', via: 'asset:D2' });
  });
});
