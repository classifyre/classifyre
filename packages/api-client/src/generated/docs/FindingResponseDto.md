...
`comment` | string
`history` | [Array&lt;FindingHistoryEntryDto&gt;](FindingHistoryEntryDto.md)
`detectedAt` | Date
...
```typescript
import type { FindingResponseDto } from '@workspace/api-client'
...
const exampleParsed = JSON.parse(exampleJSON) as FindingResponseDto
