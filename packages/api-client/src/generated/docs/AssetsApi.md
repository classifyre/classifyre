...
async function example() {
  ...
  const body = {
    ...
    assetStatus: ...,
    // Array<'JIRA' | 'CONFLUENCE' | 'CROWD' | 'BITBUCKET' | 'SERVICEDESK' | 'XRAY' | 'GOOGLE_DRIVE' | 'GOOGLE_SHEETS' | 'GOOGLE_DOCS' | 'GOOGLE_SLIDES' | 'WORDPRESS' | 'SLACK' | 'S3_COMPATIBLE_STORAGE' | 'AZURE_BLOB_STORAGE' | 'GOOGLE_CLOUD_STORAGE' | 'POSTGRESQL' | 'MYSQL' | 'MSSQL' | 'ORACLE' | 'HIVE' | 'DATABRICKS' | 'SNOWFLAKE' | 'MONGODB' | 'NEO4J' | 'SQLITE' | 'NOTION' | 'POWERBI' | 'TABLEAU' | 'EMAIL' | 'YOUTUBE' | 'DELTA_LAKE' | 'ICEBERG' | 'KAFKA' | 'ELASTICSEARCH' | 'OPENSEARCH' | 'MEILISEARCH' | 'LOCAL_FOLDER' | 'MICROSOFT_365' | 'GOOGLE_WORKSPACE' | 'CUSTOM'> (optional)
    assetSourceType: ...,
