...
export function AssetCircle({
  ...
  /** Found by a trace, not on the board: a dashed outline. */
  ghost?: boolean;
}) {
  ...
  return (
    <g>
      ...
      <circle
        ...
        r={ASSET_NODE.r - 2}
        fill="var(--background)"
        stroke={missing ? "var(--cb-contradicts)" : ghost ? "var(--muted-foreground)" : "var(--foreground)"}
        strokeWidth={1.75}
        strokeDasharray={missing ? "3 2" : ghost ? "4 3" : undefined}
      />
    </g>
  );
}
