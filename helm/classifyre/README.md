...
Object storage is **disabled by default**. When disabled, runner logs stream live during a run but are not persisted — the UI shows a warning banner on the Logs tab. Enable and configure any S3-compatible provider to retain logs.

See [`docs/object-storage.md`](docs/object-storage.md) for the full provider guide.
