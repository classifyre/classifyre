...
| `values-vps.yaml` | single-node VPS showcase (`classifyre` ns, NodePort 30100) | Public read-only instance. Deployed by CI (`Deploy Image` → `classifyre`) and `scripts/demo-deployment.sh`. The node budget is documented inside the file. |
